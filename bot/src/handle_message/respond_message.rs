...
pub async fn respond_message_room(
    ...
    if msg.event_type == "connect" {
        ...
        common::handle_connect(socket_write, "room").await;
    } else if msg.event_type == "sid" {
        // Respond to SID event
        room::handle_sid(socket_write, room_state).await;
    } else if msg.event_type == "ping" {
        // Respond to ping event
        common::handle_ping(socket_write, "room").await;
    } else if msg.event_type == "message" {
        // Handle message event
        room::handle_message(socket_write, room_state, msg).await;
    } else if msg.event_type == "roomEntry" {
        // Handle room entry event
        room::handle_entry(socket_write, room_state, msg).await;
    }
}
...
pub async fn respond_message_game(
    ...
    if msg.event_type == "connect" {
        ...
        // Respond to SID event
        game::handle_sid(socket_write, room_state, msg).await;
    } else if msg.event_type == "ping" {
        // Respond to ping event
        common::handle_ping(socket_write, "room").await;
    } else if msg.event_type == "message" {
        // Handle message event
        game::handle_message(socket_write, room_state, msg).await;
    }
}
