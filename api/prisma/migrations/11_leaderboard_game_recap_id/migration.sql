...
CREATE MATERIALIZED VIEW leaderboard AS (
    WITH all_scores AS (
        ...
        JOIN LATERAL (
            ...
            ('depleted_syllables_count', gr.depleted_syllables_count),
            ('words_without_death_count', gr.words_without_death_count),
            ('alpha_count', gr.alpha_count),
            ('previous_syllables_count', gr.previous_syllables_count),
            ('multi_syllables_count', gr.multi_syllables_count),
            ('hyphen_words_count', gr.hyphen_words_count),
            ('more_than_20_letters_words_count', gr.more_than_20_letters_words_count),
            ('slurs_count', gr.slurs_count),
            ('creatures_count', gr.creatures_count),
            ('ethnonyms_count', gr.ethnonyms_count),
            ('chemicals_count', gr.chemicals_count),
            ('plants_count', gr.plants_count),
            ('foods_count', gr.foods_count),
            ('adverbs_count', gr.adverbs_count),
            ('time', EXTRACT(EPOCH FROM (gr.died_at - g.started_at)) * 1000)
    ...
    lbct1 AS (
        ...
        "score",
        ROW_NUMBER() OVER (
            PARTITION BY "language", "mode", "record_type"
    ...
    lbct2 AS (
        ...
        CASE
            WHEN CAST(
            ROW_NUMBER() OVER (
                PARTITION BY lbct1.player_id, lbct1."language", lbct1.record_type
                ORDER BY lbct1.pp DESC, lbct1.player_id ASC
            ) AS INT
            ) = 1 THEN TRUE
            ELSE FALSE
    ...
    lbct3 AS (
        ...
        *,
        CAST(GREATEST(
            0.0,
            1.0 - ((ROW_NUMBER() OVER (
                PARTITION BY lbct2.player_id, lbct2."language", lbct2.best_pp_in_record_type
                ORDER BY lbct2.pp DESC, lbct2.player_id ASC
            ) - 1) * 0.12)
        ) AS FLOAT) AS pp_weight
        FROM lbct2
    ),
    lbct4 AS (
        SELECT *, ROUND(lbct3.pp * lbct3.pp_weight) AS weighted_pp
        FROM lbct3
    )
    SELECT * FROM lbct4
);
...
CREATE MATERIALIZED VIEW pp_leaderboard AS (
    WITH ct AS (
        SELECT DISTINCT ON ("player_id", "language", "pp_sum")
        "player_id",
        "language",
        SUM("weighted_pp") OVER (PARTITION BY player_id, "language") AS pp_sum
        FROM leaderboard l
        WHERE best_pp_in_record_type = TRUE
        ORDER BY "language", "pp_sum" DESC
    )
    SELECT
        *,
        CAST(ROW_NUMBER() OVER (PARTITION BY "language" ORDER BY "pp_sum" DESC, "player_id" ASC) AS INT) AS rank
    FROM ct
);
