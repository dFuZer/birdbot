...
fn get_letter_rarity_scores(dictionary: &[String], bonus_letters: &str) -> HashMap<char, f64> {
    ...
    let min_letters = letter_rarity_scores.values()
        .copied()
...
fn evaluate_sn_word(word: &str, syllables_count: &HashMap<String, i32>) -> f64 {
    ...
    for (syllable, count) in word_syllables {
        if let Some(&before_placing_word) = syllables_count.get(&syllable) {
            ...
            let depletion_percentage = (before_placing_word - after_placing_word) as f64 / before_placing_word as f64;
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("Usage: {} <language> <dictionary_path> <output_dir>", args[0]);
        process::exit(1);
    }
    ...
    bonus_letters_per_language.insert("br".to_string(), "abcdefghijklmnopqrstuvxyz".to_string());
    ...
    bonus_letters_per_language.insert("pt-BR".to_string(), "abcdefghijlmnopqrstuvxz".to_string());
    ...
    bonus_letters_per_language.insert("eu".to_string(), "abdefghijklmnoprstuxz".to_string());
    ...
    let metadata = serde_json::json!({
        "letter_rarity_scores": letter_rarity_scores,
        "syllables_count": syllables_count,
        "top_flip_words": top_flip_words,
        "top_sn_words": top_sn_words
    });
    ...
    let duration = start_time.elapsed();
    println!("Metadata generation completed in {:.2} seconds", duration.as_secs_f64());
    process::exit(0);
}
