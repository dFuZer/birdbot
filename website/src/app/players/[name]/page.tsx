...
export default async function Page({ params, searchParams }: { params: Promise<{ name: string }>; searchParams: TSearchParams }) {
    const [paramsAwaited, searchParamsAwaited] = await Promise.all([params, searchParams]);
    const { name: playerName } = paramsAwaited;
    ...
    const playerDataResponse = await getFromApi(
        `/player-profile?name=${playerName}${selectedLanguage ? `&language=${selectedLanguage}` : ""}${selectedMode ? `&mode=${selectedMode}` : ""}`,
    );
