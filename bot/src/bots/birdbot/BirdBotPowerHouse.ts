...
export async function loadDictionaryMetadata(
    ...
    return new Promise((resolve, reject) => {
        ...
        const powerHousePath = path.join(
            distPath,
            "bots",
            "birdbot",
            "powerhouse",
        ...
        const powerhouseProcess = spawn(powerHousePath, [
            dictionaryId,
            dictionaryFilePath,
            dataPath,
        ]);
        ...
        powerhouseProcess.on("exit", async (code) => {
            if (code === 0) {
                const dictionaryMetadataFilePath = path.join(
                    dataPath,
                    `${dictionaryId}.bbdm`
                );

                const dictionaryMetadataFileContent =
                    await Utilitary.readArrayFromFileAsync(
                        dictionaryMetadataFilePath
                    );
                const [metadataHash, dictionaryMetadataString] =
                    dictionaryMetadataFileContent;
                if (!dictionaryMetadataString) {
                    reject(
                        `Could not generate dictionary metadata for ${dictionaryId}`
                    );
                }
                const dictionaryMetadata = JSON.parse(
                    dictionaryMetadataString
                ) as CacheableDictionaryMetadata;
    ...
    return new Promise((resolve, reject) => {
        ...
        powerhouseProcess.on("exit", async (code) => {
            if (code === 0) {
                ...
                const [dictionaryResourceArray, dictionaryMetadataFileContent] =
                    await Promise.all([
                        Utilitary.readArrayFromFileAsync(dictionaryFilePath),
                        Utilitary.readArrayFromFileAsync(
                            dictionaryMetadataFilePath
                        ),
                    ]);
                const [metadataHash, dictionaryMetadataString] =
                ...
                Logger.log({
                    message: `${dictionaryId} dictionary: time to generate metadata: ${(
                        ...
                        2
                    )} milliseconds, time to generate dictionary resource: ${(
