...
export default class BirdBotUtils {
    public static handleMyTurn: BotEventHandlerFn = (ctx) => {
        ...
        if (mode === "flip") {
            const requiredLetters =
                ctx.room.roomState.gameData!.dictionaryManifest.bonusLetters;
    ...
    public static getTopFlipWords = (
        ...
        return dictionary
            .map((word) => {
                const score = this.evaluateFlipWord(
                    word,
                    letterRarityScores,
                    necessaryLetters,
                    ""
    ...
    public static evaluateSnWord = (
        ...
        for (let syllable in wordSyllables) {
            if (syllablesCount[syllable] === undefined) {
                ...
                throw new Error(
                    `Syllable ${syllable} not found in syllablesCount. This should never happen.`
                );
            }
            const beforePlacingWord = syllablesCount[syllable]!;
            const afterPlacingWord =
                beforePlacingWord - wordSyllables[syllable]!;
            const depletionPercentage =
                (beforePlacingWord - afterPlacingWord) / beforePlacingWord;
    ...
    public static handlePlayerDeath = async (
        ctx: EventCtx,
        gamerId: number
    ) => {
        ...
        const timeSurvived =
            gameRecap.diedAt -
            ctx.room.roomState.gameData!.round.startTimestamp;
    ...
    public static findValuesInAliasesObject = <T extends string>(
        values: string[],
        aliases: Record<T, string[]>
    ): T[] => {
        const targetItems: T[] = [];
    ...
    public static getApiGameRecap = (
        ...
        const player = gameData.players.find(
            (player) => player.gamerId === gamerId
        );
        if (!player) {
            throw new Error("Player not found");
        }
        const gamer = ctx.room.roomState.roomData!.gamers.find(
            (gamer) => gamer.id === player.gamerId
        );
        if (!gamer) {
    ...
    public static getApiGameData = (ctx: EventCtx) => {
        ...
        const language =
            dictionaryIdToBirdbotLanguage[
                gameData.rules.dictionaryId as BirdBotSupportedDictionaryId
            ];
        const roomMetadata = ctx.room.roomState.metadata as BirdBotRoomMetadata;
        if (!language) {
            Logger.error({
            ...
            throw new Error(
                `Language ${gameData.rules.dictionaryId} not supported. This should never happen.`
            );
        }
        return {
    ...
    public static setRoomGameMode = (
        ctx: CommandOrEventCtx,
        mode: DictionaryLessGameRules
    ) => {
    ...
    public static setRoomGameRuleIfDifferent = (
        ctx: CommandOrEventCtx,
        ...
        if (ctx.room.roomState.gameData!.rules[rule] !== value) {
            ...
            ctx.room.ws!.send(
                ctx.bot.networkAdapter.getSetupMessage(rule, value)
            );
        } else {
            Logger.log({
    ...
    public static detectRoomGameMode = (ctx: EventCtx) => {
        ...
        for (const gameModeKey in birdbotModeRules) {
            ...
            for (const ruleKey in gameMode) {
                ...
                if (
                    ctx.room.roomState.gameData!.rules[ruleKey as Rule] !==
                    ruleValue
                ) {
        ...
        if (!foundCorrespondingGameMode) {
            Logger.log({
                message:
                    "No corresponding game mode found. Setting game mode to custom.",
                path: "BirdBotEventHandlers.ts",
            });
            const roomMetadata = ctx.room.roomState
                .metadata as BirdBotRoomMetadata;
    ...
    public static getRecordsFromApi = async ({
        ...
        if (recordType) {
            return await this.getJsonFromApi<ApiResponseBestScoresSpecificRecord>(
                `/records?lang=${language}&mode=${gameMode}&page=${
                    page ?? 1
                }&perPage=5&record=${recordType}`
            );
        } else {
            return await this.getJsonFromApi<ApiResponseAllRecords>(
                `/records?lang=${language}&mode=${gameMode}`
            );
        }
    };

    public static findTargetItemInZodEnum = <T>(
        params: string[],
        schema: z.ZodSchema<T>
    ): T | null => {
        const validParam = params.find(
            (param) => schema.safeParse(param).success
        );
        if (validParam) {
    ...
    public static findBestUsernameMatch = (
        str: string,
        gamers: Gamer[]
    ): Gamer | null => {
        const perfectMatch = gamers.find(
            (gamer) => gamer.identity.nickname === str
        );
        if (perfectMatch) return perfectMatch;
        const perfectCaseInsensitiveMatch = gamers.find(
            (gamer) =>
                gamer.identity.nickname.toLowerCase() === str.toLowerCase()
        );
        ...
        const includeMatch = gamers.find((gamer) =>
            gamer.identity.nickname.toLowerCase().includes(str.toLowerCase())
        );
        if (includeMatch) return includeMatch;
        return null;
    };

    public static submitWord = ({
        adapter,
        word,
        ws,
    }: {
    ...
    public static getBestFlipWord = ({
        ...
        const letterRarityScores =
            dictionaryResource.metadata.letterRarityScores;

        const maxPossibleScore = this.evaluateFlipWord(
            requiredLetters,
            letterRarityScores,
            requiredLetters,
            placedLetters
        ...
        const randomStartIndex = Math.floor(
            Math.random() * dictionaryResource.resource.length
        );
        for (
            let i = randomStartIndex;
            i < dictionaryResource.resource.length;
            i++
        ) {
    ...
    public static splitWordIntoSyllables = (word: string) => {
        ...
        for (const subword of subwords) {
            for (
                ...
                for (
                    let letterIndex = 0;
                    letterIndex <= subword.length - syllableLength;
                    letterIndex++
                ) {
                    const syllable = subword.substring(
                        letterIndex,
                        letterIndex + syllableLength
                    );
    ...
    public static getCurrentDictionaryResource = (ctx: EventCtx) => {
        ...
        return ctx.bot.getResource<DictionaryResource>(
    ...
    public static setupRoomMetadata = (ctx: EventCtx) => {
        ...
        const currentDictionaryResource =
            this.getCurrentDictionaryResource(ctx);
    ...
    public static getFormattedPlayerScores = (
        playerStats: PlayerGameScores,
        lng: BirdBotLanguage
    ) => {
        const scores = [
