  ...
  handler: async (ctx) => {
    ...
    if (targetRecordType) {
          ...
          (score) =>
            `${score.rank}) ${t("general.scorePresentation", {
              ...
              score: t(`lib.recordType.${targetRecordType}.score`, {
                context: "specific",
                count: score.score,
                formattedScore: recordsUtils[targetRecordType].format(
                  score.score
                ),
                lng: l(ctx),
              }),
      ...
      const records = r.bestScores
        ...
        .map((score) => {
          return `${t(`lib.recordType.${score.record_type}.recordName`)}: ${t(
            "general.scorePresentation",
            {
              username: score.player_username,
              score: t(`lib.recordType.${score.record_type}.score`, {
                count: score.score,
                formattedScore: recordsUtils[score.record_type].format(
                  score.score
                ),
                lng: l(ctx),
              }),
  ...
  handler: (ctx) => {
    ...
    function sendResults(username: string, playerStats: PlayerGameScores) {
      ...
      if (scores.length > 0) {
        ...
        ctx.utils.sendChatMessage(
          t("command.currentGameScore.noScores", { username, lng: l(ctx) })
        );
      }
    }
  ...
  handler: (ctx) => {
      ...
      ctx.utils.sendChatMessage(
        t("error.searchWords.nonsensicalRecordSearch", {
          records: allParamRecords
            .map((record) =>
              t(`lib.recordType.${record}.recordName`, { lng: l(ctx) })
            )
            .join(", "),
          lng: l(ctx),
        })
    ...
    if (requestedSortRecords.includes("flips")) {
      ...
      if (ctx.args.length > 1) {
        ctx.utils.sendChatMessage(
          t("error.searchWords.mustProvideOneSyllable", { lng: l(ctx) })
        );
        return;
      }
...
const playerProfileCommand = c({
  ...
  handler: async (ctx) => {
    ...
    const records = playerData.records
      .sort(
        (a, b) =>
          recordsUtils[a.record_type].order - recordsUtils[b.record_type].order
      )
      .map((record) => {
...
const testCommand = c({
  ...
  handler: (ctx) => {
    ...
    ctx.utils.sendChatMessage(
      dict.metadata.testWords.map((x) => x.word).join(" ")
    );
  },
}) satisfies Command;
...
const discordCommand = c({
  ...
  handler: (ctx) => {
    ctx.utils.sendChatMessage(
      t("command.discord.result", { link: DISCORD_SERVER_LINK, lng: l(ctx) })
    );
  },
}) satisfies Command;
...
const donateCommand = c({
  ...
  handler: (ctx) => {
    ctx.utils.sendChatMessage(
      t("command.donate.result", { link: PAYPAL_DONATE_LINK, lng: l(ctx) })
    );
  },
}) satisfies Command;
...
const createRoomCommand = c({
  ...
  handler: (ctx) => {
    ...
    (ctx.bot.rawBot as BirdBot).createRoom({
      targetConfig: {
        dictionaryId:
          birdbotLanguageToDictionaryId[targetLanguage ?? defaultLanguage],
        gameMode: "survival",
