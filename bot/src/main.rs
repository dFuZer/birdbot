mod get_socket;
mod handle_socketio_message;

use std::sync::Arc;
use tokio::sync::Mutex;

use env_logger;
use futures_util::StreamExt;
use get_socket::get_socket;
use handle_socketio_message::{handle_message_game, handle_message_room};

pub const JKLM_ROOM_URL: &str = "wss://phoenix.jklm.fun/socket.io/?EIO=4&transport=websocket";

struct RoomState {
    room_connected: bool,
    game_connected: bool,
    room_code: String,
    user_token: String,
    profile_pic: String,
}

#[tokio::main]
async fn main() {
    env_logger::init();

    let room_state = Arc::new(Mutex::new(RoomState {
        room_connected: false,
        game_connected: false,
        room_code: "CAYP".to_string(),
        user_token: "K3GJW268IxM4cH22".to_string(),
        profile_pic: "".to_string(),
    }));

    let (mut write_room, mut read_room) = get_socket(JKLM_ROOM_URL).await.split();

    // Read room messages
    let room_handle = {
        let room_state_clone = Arc::clone(&room_state);
        tokio::spawn(async move {
            while let Some(msg) = read_room.next().await {
                match msg {
                    Ok(msg) => handle_message_room(msg, &mut write_room, &room_state_clone).await,
                    Err(e) => println!("[room socket error] {:?}", e),
                }
            }
        })
    };

    // Read game messages
    let game_handle = {
        let room_state_clone = Arc::clone(&room_state);

        tokio::spawn(async move {
            // Wait for notification
            loop {
                // Check the condition under the lock

                let state = room_state_clone.lock().await;
                if state.room_connected {
                    break;
                }

                // Wait for the notify signal
            }

            // Room is connected, now create the game socket
            println!("[room connected, creating game socket]");
            let (mut write_game, mut read_game) = get_socket(JKLM_ROOM_URL).await.split();

            // Read game messages
            while let Some(msg) = read_game.next().await {
                match msg {
                    Ok(msg) => handle_message_game(msg, &mut write_game, &room_state_clone).await,
                    Err(e) => println!("[game socket error] {:?}", e),
                }
            }
        })
    };

    // Task to log the room_state object every 5 seconds
    /* let _ = {
        let room_state_clone = Arc::clone(&room_state);
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(5));
            loop {
                interval.tick().await;

                println!(
                    "RoomState: {}",
                    room_state_clone.lock().await.room_connected
                );
            }
        })
    }; */

    // Wait for both tasks to complete
    let _ = tokio::try_join!(game_handle, room_handle);
}
