...
const resources = {
    en: {
        error: {
            "intro": "Error:",
            "unspecific": "$t(intro) An unknown error occurred.",
            "api": {
                inaccessible: "$t(intro) The API is inaccessible. Please try again later.",
            },
            "roomState": {
                noGameInProgress: "$t(intro) No game in progress.",
                notInPregame: "$t(intro) Not in pregame.",
                notEnoughPlayers: "$t(intro) Not enough players to start the game.",
                cannotSetMode: "$t(intro) Cannot set mode outside of pregame.",
                cannotSetLanguage: "$t(intro) Cannot set language outside of pregame.",
            },
            "invalid": {
                regex: "$t(intro) Invalid regex: {{regex}}",
                language: "$t(intro) Invalid language.",
                gameMode: "$t(intro) Invalid game mode.",
            },
            "notSupported": {
                language: '$t(intro) Language "{{language}}" is not supported.',
            },
            "404": {
                word: "$t(intro) Word not found in dictionary.",
                player: "$t(intro) Player not found.",
                gamer: "$t(intro) Gamer not found in room. This should not happen.",
                dictionaryResource: "$t(intro) Could not find dictionary resource for this language. This should not happen.",
                playerStats: "$t(intro) Player stats not found for this player. This should not happen.",
                currentPlayer: "$t(intro) No current player.",
                syllableNotExists: "$t(intro) This syllable does not exist in any word in the requested dictionary.",
            },
            "searchWords": {
                nonsensicalRecordSearch: "$t(intro) It makes no sense to sort words by record(s): {{records}}.",
                multipleSortRecords:
                    "$t(intro) You can only sort by one sort-record at a time. The sort-records are: {{sortRecords}}. Note that you can still filter by multiple filter-records. The filter-records are: {{filterRecords}}.",
                mustProvideOneSyllable: "$t(intro) You can only sort by multi-syllable words if you provide exactly one syllable.",
            },
            "invalidParams": {
                noUsername: "$t(intro) You must provide a username to use this command.",
                mustProvideWord: "$t(intro) You must provide a word to use this command.",
            },
            "platform": {
                mustBeLoggedIn:
        ...
        command: {
            ...
            currentGameScore: {
                description: "Shows the current game scores for a given player. If no player is provided, it will show the scores for the current player.",
        ...
        utils: {
            playerStats: {
                diedNoWords: "Oh no, {{username}} died without placing any words this game. Better luck next time!",
        ...
        eventHandler: {
            chat: {
                ...
                notRoomCreator: "You cannot use this command if you are not the room creator. /b to create your room will be available soon.",
            },
            submit: {
